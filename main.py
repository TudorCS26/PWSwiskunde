...
lives = MAX_LIVES
BALL_BASE_SPEED = 6
BALL_MAX_SPEED = 12  # Maximale snelheid van de bal

ball_paused = False
game_over = False

# Voeg kleine random afwijking toe aan balrichting
...
def initialize_game():
    ...
    global bricks_img_scaled, score, lives, game_over
    ...
    ball_speed_y = BALL_BASE_SPEED * math.pi
...
# Paddle frames
paddle_imgs = []
paddle_coords = [(1158, 462, 243, 64), (1158, 528, 243, 64), (1158, 594, 243, 64)]
...
def start_screen():
    ...
    while True:
        ...
        screen.fill((0,0,0))
        draw_text('Voer rijen (6-20):', input_font, 'white', screen, 100, 100)
        draw_text('Voer kolommen (rijen+2):', input_font, 'white', screen, 100, 200)
...
if start_screen():
    ...
    while running:
        game_current_score_img = font.render(f"Score = {score}", False, 'green')
        ...
        if not ball_paused and not game_over:
            ...
            # Paddle collision
            if (ball_speed_y > 0 and ball_y + BALL_HEIGHT <= paddle_y and
            ...
            # Brick collision en snelheid verhogen
            brick_hit = False
                ...
                for j, brick_alive in enumerate(row):
                    if brick_alive:
                        ...
                        if brick_rect.colliderect(ball_rect):
                            bricks[i][j] = False
                            score += 10
                            # Voeg kleine random afwijking toe
                            ball_speed_x, ball_speed_y = add_random_angle(ball_speed_x, ball_speed_y)
                            # Verhoog snelheid met 5%
                            speed = math.hypot(ball_speed_x, ball_speed_y) * 1.05
